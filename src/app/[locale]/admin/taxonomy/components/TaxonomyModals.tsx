...
export function TaxonomyModals({ adminTaxonomy }: TaxonomyModalsProps) {
  const {
    itemToDelete,
    setItemToDelete,
    deleteMutation,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: '6px 0 0 0', fontSize: '0.875rem', lineHeight: '1.4', color: 'var(--admin-text-secondary, #6b564c)' }}>
                  {isVi
                    ? `Bạn có chắc chắn muốn xóa vĩnh viễn ${selectedIds.length} mục đã chọn? Thao tác này không thể hoàn tác.`
                    : `Are you sure you want to permanently delete the ${selectedIds.length} selected items? This cannot be undone.`}
                </p>
              </div>
            </div>
            ...
            <div style={{
              ...
              {Array.from(selectedItemNames.entries()).filter(([id]) => selectedIds.includes(id)).map(([id, name]) => (
