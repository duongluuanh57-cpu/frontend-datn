...
interface TaxonomyBulkActionBarProps {
  adminTaxonomy: UseAdminTaxonomyReturn;
}

export function TaxonomyBulkActionBar({ adminTaxonomy }: TaxonomyBulkActionBarProps) {
  const {
    selectedIds,
    isVi,
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#f0e9e4' }}>
        {isVi ? `Đã chọn ${selectedIds.length} mục` : `${selectedIds.length} items selected`}
      </span>
