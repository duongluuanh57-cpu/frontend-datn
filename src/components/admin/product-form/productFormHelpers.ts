...
export function toFormState(data?: ProductFormData): ProductFormData {
  if (!data) {
    return {
      ...
      discountPercentage: 10, discountStartDate: null as Date | null,
  ...
  return {
    ...
    season: data.season ?? '', time: data.time ?? '',
    ...
    longevity: data.longevity ?? '', sillage: data.sillage ?? '',
    durability: data.durability ?? '', scentTrail: data.scentTrail ?? '',
    style: data.style ?? '',
    suitableFor: data.suitableFor ?? '', occasion: data.occasion ?? '',
