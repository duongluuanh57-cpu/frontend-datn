...
export const slugify = (text: string): string => {
  ...
  const slug = text
    ...
    .toLowerCase()
    .trim()
    // Thay thế ký tự tiếng Việt trước
  ...
  // Nếu slug rỗng sau khi xử lý, dùng timestamp làm fallback
...
`.trim();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Lưu lại page hiện tại để sau khi redirect về list vẫn đúng trang
      if (productId && typeof window !== 'undefined') {
        const currentSaved = sessionStorage.getItem('adminProductListCurrentPage');
        if (currentSaved) {
          sessionStorage.setItem('adminProductListPage', currentSaved);
        }
      }
