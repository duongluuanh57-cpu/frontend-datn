...
import { ChatProductCard, type ProductData } from './ChatProductCard';
...
export function ChatWidgetMessageList({ formHelpers }: ChatWidgetMessageListProps) {
  ...
  const renderMessageContent = (content: string) => {
    ...
    // Parse text to detect URLs and convert to clickable links, preserve line breaks
        ...
        if (i > 0 && matches[i - 1]) {
          result.push(
            <a
              ...
              className="text-[#C48B8B] underline hover:text-[#a06c6c] transition-colors"
            >
              {matches[i - 1]}
            </a>
          );
    ...
    return (
      <div className="flex flex-col gap-2">
        {cleanText && (
          <span className="leading-relaxed text-sm whitespace-pre-wrap">
        ...
        <div key={msg.id} className={cn("message-wrapper", msg.role === 'user' ? 'user' : 'assistant')}>
          ...
          {msg.role === 'assistant' && msg.id !== 'welcome' && !isLoading && (
            <ChatFeedback 
              ...
              // Câu hỏi của user ngay trước tin nhắn AI này
              userMessage={(() => {
                // Lấy tin nhắn user gần nhất trước tin nhắn AI này
                for (let i = index - 1; i >= 0; i--) {
                  if (messages[i].role === 'user') return messages[i].content;
                }
                return '';
              })()}
              // Nội dung câu trả lời AI
