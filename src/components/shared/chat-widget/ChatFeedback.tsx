...
interface ChatFeedbackProps {
  ...
  imagePath?: string;
  /** Câu hỏi gốc của user (để lưu cache) */
  userMessage?: string;
  /** Câu trả lời của AI (để lưu cache) */
  aiResponse?: string;
}

const RATING_LABELS: Record<number, { label: string; emoji: string; color: string }> = {
  5: { label: 'Tuyệt vời!',       emoji: '🌟', color: '#C48B8B' },
...
export function ChatFeedback({ messageId, initialRating, onRatingSubmit, imagePath, userMessage, aiResponse }: ChatFeedbackProps) {
              ...
              style={{
                fill:
                  activeRating !== null && star <= activeRating
                    ? (RATING_LABELS[activeRating]?.color ?? '#C48B8B')
                    : 'transparent',
                stroke:
                  activeRating !== null && star <= activeRating
                    ? (RATING_LABELS[activeRating]?.color ?? '#C48B8B')
