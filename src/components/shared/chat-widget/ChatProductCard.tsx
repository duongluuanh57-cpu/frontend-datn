...
export function ChatProductCard({ product, loading }: ChatProductCardProps) {
  if (loading) return (
    <div className="w-full aspect-[4/5] animate-pulse bg-white/5 rounded-xl border border-white/10 flex items-center justify-center">
      <Sparkles size={12} className="text-[#C48B8B] animate-spin" />
    </div>
  );
  ...
  return (
    <Link href={`/product/${product._id}`}>
      <motion.div 
        ...
        className="w-full flex flex-col overflow-hidden bg-white rounded-2xl border border-[#2D2D2D]/10 cursor-pointer transition-all shadow-sm"
      >
        ...
        <div className="p-2.5 flex flex-col gap-0.5">
          ...
          <p className="text-[10px] font-black text-[#C48B8B]">
